...
namespace Norm
{
    public partial class Norm
    {
        ...
        public IEnumerable<T> Read<T>(string command, params (string name, object value, object type)[] parameters)
        {
            ...
            if (t1.valueTuple)
            {
                return ReadToArrayInternalUnknowParamsType(command, parameters).MapValueTuple<T>(t1.type);
            }
            if (!t1.simple)
            {
                return ReadToArrayInternalUnknowParamsType(command, parameters).Map<T>(t1.type);
            }

            return ReadInternalUnknowParamsType(command, r => GetFieldValue<T>(r, 0, t1.type), parameters);
        }
            ...
            params (string name, object value, object type)[] parameters)
        {
            ...
            if (t1.valueTuple)
            {
                return ReadToArrayInternalUnknowParamsType(command, readerCallback, parameters).MapValueTuple<T>(t1.type);
            }
            if (!t1.simple)
            {

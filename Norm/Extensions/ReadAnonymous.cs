...
#pragma warning restore CS1573 // Parameter has no matching param tag in the XML comment (but other parameters do)
            where T : class
        {
            return connection.GetNoOrmInstance().Read<T>(anonymousBlueprintInstance, command, readerCallback, memberName, sourceFilePath, sourceLineNumber);
        }
...
#pragma warning restore CS1573 // Parameter has no matching param tag in the XML comment (but other parameters do)
            where T : class
        {
            return connection.GetNoOrmInstance().ReadFormat<T>(anonymousBlueprintInstance, command, readerCallback, memberName, sourceFilePath, sourceLineNumber);
        }
    }
}
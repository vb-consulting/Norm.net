...
#pragma warning restore CS1573 // Parameter has no matching param tag in the XML comment (but other parameters do)
        {
            return connection.GetNormInstance().Read<T1, T2, T3, T4>(command, memberName, sourceFilePath, sourceLineNumber);
        }
...
#pragma warning restore CS1573 // Parameter has no matching param tag in the XML comment (but other parameters do)
        {
            return connection.GetNormInstance().ReadFormat<T1, T2, T3, T4>(command, memberName, sourceFilePath, sourceLineNumber);
        }
    }
}
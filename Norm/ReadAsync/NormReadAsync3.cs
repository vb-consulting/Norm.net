...
namespace Norm
{
    public partial class Norm
    {
        ...
        public IAsyncEnumerable<(T1, T2, T3)> ReadFormatAsync<T1, T2, T3>(FormattableString command)
        {
            ...
            if (t1.valueTuple && t2.valueTuple && t3.valueTuple)
            {
                return ReadFormatAsync(command).MapValueTuple<T1, T2, T3>(t1.type, t2.type, t3.type);
            }
            else if (!t1.simple && !t2.simple && !t3.simple)
            {
                return ReadFormatAsync(command).Map<T1, T2, T3>(t1.type, t2.type, t3.type);
            }
            return ReadInternalAsync(command, async r => (
                await GetFieldValueAsync<T1>(r, 0, t1.type),
                await GetFieldValueAsync<T2>(r, 1, t2.type),
            ...
            params (string name, object value, object type)[] parameters)
        {
            ...
            if (t1.valueTuple && t2.valueTuple && t3.valueTuple)
            {
                return ReadToArrayInternalUnknownParamsTypeAsync(command, parameters).MapValueTuple<T1, T2, T3>(t1.type, t2.type, t3.type);
            }
            else if (!t1.simple && !t2.simple && !t3.simple)
            {
                return ReadToArrayInternalUnknownParamsTypeAsync(command, parameters).Map<T1, T2, T3>(t1.type, t2.type, t3.type);
            }

...
Snake case naming. Query results containing case name fields will be mapped properly to C# names.
For example query:
select * from (
values 
...
Those fields will map properly to you C# class by using standard call var result = connection.Read(sql).Select<SnakeCaseMapTestClass>();:
...
Array field support. Some servers like PostgreSQL can return array as result which simplifies programming model and improves perfomances as well.
